...
const { ranked } = require('../db/channels.json');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('ranked')
        .setDescription('sends info for the current and upcoming ranked battles'),
    async execute(interaction) {
        await interaction.reply('command currently wip');
        /*
        const response = await GetSchedule('32449507786579989234');

        const embeds = getRankedEmbeds(response, 5);

        interaction.deferReply();
        interaction.deleteReply();
        await interaction.guild.channels.cache.get(ranked).send({ embeds: embeds[0] });
        /*  .then(msg => {
                setTimeout(() => {
                    msg.delete();
